#' lines2pts -- Lines to Points
#'
#' A function that calculates the total length of osm_lines, that intersect the neighbourhood of provided points. Data is derived from OpenStreetMaps database.
#'
...
lines2pts<-function(key, value, pts, radius, sleep=1, binary=F, multi=T){
  ...
  for(i in 1:length(key)) {
    ...
    if(dim(lines)[1]>0)
    {
      ...
      id<-st_intersects(lines, circles) %>% unlist()
      length<-st_intersection(lines, circles) %>% st_length()

#' poly2poly_i -- Polygons to Polygons: intersection
#'
#' A function that calculates the total number of `osm_polygons`, that intersect with provided polygons. Data is derived from OpenStreetMaps database.
#'
...
poly2poly_i<-function(key, value, region, sleep=1, binary=F, multi=T, minsize=0){
  ...
  for(i in 1:length(key)) {
    ...
    if(length(poly)>0)
    {
      ...
      a<-st_intersects(region.sf, poly)

#' pts2poly -- Points to Polygons
#'
#' A function that calculates the total number of points located within provided polygons. Points are derived from OpenStreetMaps database.
#'
...
#' @return A data frame object. Each row represents one polygon provided by the region argument. Each column is one aggregated OpenStreetMaps feature.
...
pts2poly<-function(key, value, region, sleep=1, binary=F, outside=F, multi=F){
  ...
  for(i in 1:length(key)) {
    ...
    if(dim(variable.sf$osm_points)[1]>0)
    {
      ...
      with_list<-st_within(points, region.sf)
